...
> **Classification rationale:** These handle-retrieval and status-query APIs are validated either by a read-consistency check (the value is read twice and confirmed equal), by a return-code check, or by registering an event callback. No value is written and read back, and no comparison is made against the device profile — hence **Functional**. The two L2 tests confirm the expected source-device status (display not connected, port not active, HDCP unpowered) via the same query APIs.
...
> **Classification rationale:** These are write/enable-only operations whose L1 tests verify only the return code (`dsERR_NONE` on supported source devices, or `dsERR_OPERATION_NOT_SUPPORTED` where the operation is unsupported). No value is read back and no profile comparison is made — hence **Functional**.
...
> **Classification rationale:** Each L2 test performs a Set→Get→verify round-trip — a value is written via the Set API and the corresponding Get/status API confirms the written value survived the round-trip. This write-then-read-back pattern is **Data Integrity**. The paired L1 Set positive tests are grouped with their L2 counterparts under the same capability.
...
> **Classification rationale:** Each test retrieves a capability or setting and cross-checks the returned value against the device profile configuration (`gDSVideoPortConfiguration[]` fields such as `DisplaySurround`, `SurroundMode`, `hdcp_supported`, `hdcp_protocol_version`, `hdr_capabilities`, `Supported_tv_resolutions_capabilities`, `matrix_coefficients`, `colorspaces`, `quantization_ranges`, color depth) — or against the profile-defined default value expected for the current display-connection state. Comparison against the device profile makes this **Profile Compliance**.
