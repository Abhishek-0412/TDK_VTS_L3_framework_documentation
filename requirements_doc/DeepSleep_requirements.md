...
> **Classification rationale:** The Set/Wakeup/Get APIs are exercised for return-code correctness and valid output values. The L2 tests enter a real timed deep sleep and verify the reported wakeup reason is `DEEPSLEEP_WAKEUPREASON_TIMER` — a behavioural workflow, not a read-back of a previously written value — hence **Functional**.
