...
> **Classification rationale:** The address-retrieval, callback-registration, and transmission APIs are exercised for return-code correctness and valid output values. The L2 tests read the HAL-allocated physical/logical addresses (verifying valid ranges) and transmit broadcast/unicast CEC frames (verifying `HDMI_CEC_IO_SUCCESS` or `HDMI_CEC_IO_SENT_BUT_NOT_ACKD`) — behavioural workflows, not read-back of a written value — hence **Functional**.
