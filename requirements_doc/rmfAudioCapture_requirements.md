...
> **Classification rationale:** The default-settings retrieval, start, stop, status, and current-settings APIs are exercised for return-code correctness and populated output structures. The L2 test drives an end-to-end capture session and verifies non-zero audio data is delivered via the data-ready callback — a behavioural data-flow workflow — hence **Functional**.
