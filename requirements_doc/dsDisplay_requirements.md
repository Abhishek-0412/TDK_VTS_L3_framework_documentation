...
> **Classification rationale:** These APIs retrieve the display handle, EDID data, aspect ratio, and register the display-event callback, validated for return-code correctness and expected default values. `TestDefaultAspectRatio_src` verifies the default aspect ratio (`dsVIDEO_ASPECT_RATIO_16x9`) — a fixed-value behavioural check, not a read-back of a written value — hence **Functional**.
...
> **Classification rationale:** Each get/set API is exercised for return-code correctness, with getters verifying the expected default value. No value is written and independently read back within a single L1 test, so these are **Functional**.
...
> **Classification rationale:** This test writes each valid AVI content type via `dsSetAVIContentType()` and reads it back via `dsGetAVIContentType()`, verifying the retrieved value equals the value written — a genuine write-then-read-back round-trip — hence **Data Integrity**.
