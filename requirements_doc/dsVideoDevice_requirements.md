...
> **Classification rationale:** These APIs retrieve the video-device handle, apply a DFC zoom mode, query HDR/format/codec capabilities, and force-disable HDR — all validated for return-code correctness in isolation without cross-checking against the profile — hence **Functional**.
...
> **Classification rationale:** These APIs get/set the Frame Rate Following (FRF) mode, get the current display framerate, set the display framerate, and register the framerate pre/post change callbacks — validated for return-code correctness — hence **Functional**.
...
> **Classification rationale:** `dsGetDFC_positive` verifies the retrieved zoom mode is one of the supported DFC values declared in the device profile. The L2 tests cross-check retrieved HDR capabilities, supported video coding formats, and video codec information against the corresponding profile values — hence **Profile Compliance**.
...
> **Classification rationale:** This test writes each supported DFC value via `dsSetDFC()` and reads it back via `dsGetDFC()`, verifying the retrieved value equals the value written — a genuine write-then-read-back round-trip — hence **Data Integrity**.
